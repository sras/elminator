...
data PolyConfig = Mono | Poly deriving (Show)
...
type Constructors
  = (NE.NonEmpty ConstructorDescriptor)
  ...
  case mtName of
    Just tName -> do
      ...
      let
        ...
        polyCons = foldr
          (mkPolyMorphicConstructor constrs)
          hcons
          tVars
      pure $ HUDef $ UDefData
        (MData tnString a b)
...
renderHType_ h@(HUDef u) ip = 
  if ip
    then if DL.length (udefdTypeArgs u )> 0
      ...
      Just tvars -> T.intercalate " "  $ (_mTypeName $ udefdMdata u):(DL.zipWith renderTypeParam (udefdTypeArgs u) tvars)
      Nothing -> T.intercalate " "  $ (_mTypeName $ udefdMdata u):(renderHType <$> (udefdTypeArgs u))
  where
...
toTypeDescriptor ht@(HUDef (udef@(UDefData mdata@(MData tnString _ _) (_:_) (Just tv) x))) =
  Polymorphic tv $
    let
      rendered = (renderHType $ HUDef $ udef { udefdTypeVars = Nothing} )
      renderedPhantom = (renderHType ht)
    in case x of
      [] -> CTEmpty (TypeName tnString rendered renderedPhantom mdata)
      (a:as) -> CTData (TypeName tnString rendered renderedPhantom mdata) $ toConstructors (a:|as)
toTypeDescriptor ht@(HUDef (UDefData mdata@(MData tnString _ _) _ _ x)) =
  SimpleType $
    let
      rendered = (renderHType ht)
    in case x of 
      [] -> CTEmpty (TypeName tnString rendered rendered mdata)
      (a:as) -> CTData (TypeName tnString rendered rendered mdata) $ toConstructors (a:|as)
...
typeDescriptorToDecoder opts td = case td of
...
mkContentDecoder cd opts =
  case cd of
    ...
    RecordConstructor _cname nf -> CDRecord $ NE.toList $ NE.map modifyFieldLabel nf
